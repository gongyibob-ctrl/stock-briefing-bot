...
# baostock 已在 brief.py 顶部做 _bs_login 管理，这里直接调用同名函数
# 若独立测试，需自行确保 bs.login()


def fetch_fundamentals(code: str) -> dict:
    """
    最新年报（或可用最近季报）的 ROE / 净利润 / 净利润同比
    年报披露时点：3-4 月集中披露上年年报，5 月起多数已有。
    优先用上一自然年的年报；如果未披露则回退最新一个季报。
    """
    bsc = _bs_code(code)
    out = {}
...
def format_sector_section(sectors: list[dict]) -> str:
    ...
    lines = [f"### 🔥 今日最强行业板块（{src_label}，Top {len(sectors)}）", ""]
    # 板块概览表
    lines.append("| 板块 | 涨幅 | 净流入(亿) | 涨/跌 | 领涨 |")
    lines.append("|---|---|---|---|---|")
    for s in sectors:
        ni = s.get("net_inflow_亿")
        ni_str = f"{ni}" if ni is not None else "—"
        lines.append(
            f"| **{s['name']}** | {s['pct']:+.2f}% | {ni_str} | "
            f"{s.get('up_count','?')}↑/{s.get('down_count','?')}↓ | "
            f"{s.get('leader_name','')} {s.get('leader_pct','?')}% |"
        )
    lines.append("")
